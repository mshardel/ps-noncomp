...
# with internal pilot data using Equation 3 described in the supplement to: 
#
...
######################################################################################
# ss.noncomp.inpilot - function to compute sample size for difference in means of 
#              continuous outcomes between 2 groups with all-or-none noncompliance and
#              selection bias using internal pilot data.
######################################################################################

ss.noncomp.inpilot <- function (delta = NULL, sigma.0p.sq = NULL, sigma.1p.sq = sigma.0p.sq, 
                                rho.cp = NULL, n0.p = NULL, n1.p = n0.p, n0.s.min = NULL,  
                                                                               ...
                                                                               "one.sided"))
{
  ...
  N0 <- max((ta + tb)^2 * (sigma.0p.sq + sigma.1p.sq/ss.ratio) / delta.star^2 + 1,
            n0.p + n0.s.min)
  ...
  output <- list(n0 = ceiling(N0), n1 = ceiling(ss.ratio * 
                 ...
                 n0.p = n0.p, n1.p=n1.p, n0.s.min = n0.s.min, ss.ratio=ss.ratio,
                 sig.level = sig.level, 
